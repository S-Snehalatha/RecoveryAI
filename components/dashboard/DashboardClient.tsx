...
export function DashboardClient({ total, initialProcessed = 0 }: Props) {
  ...
  async function runBatch() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || 'Recovery batch failed.');
      setProcessed(total);
      setDone(true);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Unable to run recovery batch.');
    } finally {
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-5 shadow-sm">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-semibold text-white">Recovery batch</p>
          <p className="mt-1 text-xs text-slate-500">Run the governed recovery engine against the seeded transaction set.</p>
        </div>
        <button onClick={runBatch} disabled={running || total === 0} className="inline-flex items-center justify-center gap-2 rounded-xl bg-sky-400 px-4 py-2.5 text-sm font-semibold text-slate-950 transition hover:bg-sky-300 disabled:cursor-not-allowed disabled:opacity-50">
      ...
      {(running || done || error) && (
        <div className="mt-5 border-t border-slate-800 pt-5">
          <div className="mb-3 flex items-center justify-between text-xs">
            <span className="font-mono text-slate-400">{processed} / {total} processed</span>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
            <BatchState icon={<CheckCircle2 className="h-3.5 w-3.5" />} label="Allowed" active={running || done} />
            <BatchState icon={<UserRound className="h-3.5 w-3.5" />} label="Human Review" active={running || done} />
            <BatchState icon={<ShieldAlert className="h-3.5 w-3.5" />} label="Blocked" active={running || done} />
            <BatchState icon={<RotateCcw className="h-3.5 w-3.5" />} label="Recovered" active={done} />
          </div>
          {error && <p className="mt-3 rounded-lg border border-rose-900/60 bg-rose-950/30 px-3 py-2 text-xs text-rose-300">{error}</p>}
          {done && !error && <p className="mt-3 text-xs text-emerald-400">Batch completed. Refresh the dashboard to view persisted outcomes.</p>}
        </div>
      )}
    </div>
  );
