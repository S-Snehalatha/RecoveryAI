...
function getPendingReview(transactionId: string): HumanReview | undefined {
  return inMemoryStore
    .getHumanReviews()
    .find((review) => review.transaction_id === transactionId && review.review_status === 'PENDING');
}

/**
 * Agent brain only: investigate, diagnose, recommend, policy-check and ask
 * for human authorization. This function NEVER executes a recovery action.
 */
export async function runRecoveryAgent(transaction: Transaction): Promise<AgentRunOutput> {
  ...
  const existingPolicy = inMemoryStore
    .getPolicyDecisions(transaction.id)
    .find((policy) => policy.ai_decision_id === decision.id);
  ...
  recordAuditEvent({
    ...
    actor_id: decision.model_name,
    transaction_id: transaction.id,
    what: 'Agent diagnosed the revenue risk and generated a recovery recommendation.',
    why: decision.concise_rationale,
    ...
    state_after: {
      ...
      confidence_score: decision.confidence_score,
      expected_recovery_value: decision.raw_llm_response?.expected_recovery_value ?? null,
    },
  });
  ...
  recordAuditEvent({
    ...
    what: 'Agent recommendation was checked against deterministic recovery policy.',
    why: policyEvaluation.reason,
    result: policyEvaluation.decision,
    action_taken: decision.recommended_strategy,
    amount_in_inr: transaction.amount_in_inr,
  ...
  if (policyEvaluation.decision === 'BLOCK') {
    inMemoryStore.updateTransaction(transaction.id, { status: 'blocked' });
    recordAuditEvent({
      ...
      transaction_id: transaction.id,
      rule_id: policy.rule_id,
      what: 'Recovery action was blocked.',
      why: policyEvaluation.reason,
      result: 'BLOCKED',
      action_taken: decision.recommended_strategy,
      amount_in_inr: transaction.amount_in_inr,
  ...
  const review = existingReview ?? inMemoryStore.insertHumanReview({
    ...
    trigger_reason:
      policyEvaluation.decision === 'ALLOW'
        ? 'Agent recommendation is policy-allowed, but financial execution requires explicit human authorization.'
  ...
  if (!existingReview) {
    inMemoryStore.updateTransaction(transaction.id, {
      status: policyEvaluation.decision === 'ALLOW' ? 'review' : 'NEEDS_HUMAN_REVIEW',
    });
    recordAuditEvent({
      ...
      transaction_id: transaction.id,
      rule_id: policy.rule_id,
      what: 'Agent paused before financial execution and requested human authorization.',
      why: review.trigger_reason,
  ...
  return {
    ...
    status: policyEvaluation.decision === 'ALLOW' ? 'PENDING_APPROVAL' : 'HUMAN_REVIEW',
    decision,
    policy,
    review,
  };
